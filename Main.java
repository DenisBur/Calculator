...
public class Main {
    ...
    public static void start(){
        try {
            ...
            if(instance.matches("[0-9IVX]+[-+*/][0-9IVX]+")){
                ...
                if((0 <= firstOperand && firstOperand <= 10) && (0 <= secondOperand && secondOperand <= 10)){
                    if (isArabNum(instance)){
                        ...
                        for (char c : instance_char) { //Calculation for rom numbers
                            switch (c) {
                                case '+': System.out.println(convertArabToRom(firstOperand+secondOperand));
                                break;//Plus

                                case '-':
                                    if(firstOperand > secondOperand){
                                        System.out.print(convertArabToRom(firstOperand-secondOperand));
                                        break;//Minus
                                    }
                                    else throw new Exception("Не может быть 0 или отрицательное значение"); //

                                case '*':
                                    System.out.println(convertArabToRom(firstOperand*secondOperand));
                                    break;//Multiply
                ...
                }else{throw new Exception("Введите числа от 1 до 10");}
            }else{throw new Exception("Не коректное выражение ");}
    ...
    public static boolean isArabNum(String instance) throws Exception { //Arab = True; Roman = False; Two systems = Exception
        if(instance.matches("[0-9]+[-+*/][0-9]+")){
            ...
            return false;
        }else{throw new Exception("Нельзя считать разные числовые системы");}
    ...
    public static int convertRomToArab(String romNumber) throws Exception { // X --> 10
        ...
        }else throw new Exception("Введите числа от I до X");
    }
